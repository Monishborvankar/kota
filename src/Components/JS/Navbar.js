...
const Navbar = () => {
    ...
    return (
        <div className="Navbar" id="Navbar">
            <Link to='Home' className="nav-logo" style={{color:"white",right:"10px",padding:"20px"}}><span >COTA</span></Link>
            <div className={`nav-items ${isOpen && "open"}`}>
                <NavLink to="/" onClick={() => setIsOpen(!isOpen)}>Home </NavLink>
                <Link to="About" onClick={() => setIsOpen(!isOpen)} style={{color:"white"}}>About </Link>
